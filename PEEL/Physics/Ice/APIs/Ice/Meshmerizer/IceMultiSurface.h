...
#define ICERENDERSURFACE_H
	...
	class MESHMERIZER_API MultiSurface : public IndexedSurface
	{
		...
		virtual						bool				DiscardFaceProperties();
			// Resets the surface
		overriden(VertexCloud)		bool				Reset();

		// Surface operations
			// Keeps the face! (french humour)
		overriden(IndexedSurface)	bool				KeepFaces(const Container& faces);
			// Optimization
		overriden(IndexedSurface)	bool				Optimize();
		overriden(IndexedSurface)	bool				WeldVertices(float threshold=0.1f);
			// Subdivision
		overriden(IndexedSurface)	bool				Subdivide(SubdivScheme& scheme, bool use_cache=true, SubdivHistory* history=null);
			...
			// Flips the surface
		overriden(IndexedSurface)	bool				Flip();
