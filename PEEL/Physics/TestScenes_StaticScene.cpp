...
IMPLEMENT_STATIC_MESH_SCENE(TessellatedTerrain, gDesc_Terrain, "Terrain.bin", 1)
...
IMPLEMENT_STATIC_MESH_SCENE(TessellatedVenus, gDesc_Venus, "Venus.bin", 1)
	virtual void	GetSceneParams(PINT_WORLD_CREATE& desc)
	{
		TestBase::GetSceneParams(desc);
		desc.mCamera[0] = CameraPose(Point(14.38f, 17.68f, -17.02f), Point(-0.73f, -0.36f, 0.58f));
	}
END_TEST(TessellatedVenus)
...
END_TEST(RanRan)
IMPLEMENT_STATIC_MESH_SCENE(TessellatedRanRan, gDesc_RanRan, "RanRan.bin", 1)
	virtual void	GetSceneParams(PINT_WORLD_CREATE& desc)
	{
		TestBase::GetSceneParams(desc);
		desc.mCamera[0] = CameraPose(Point(-18.72f, 57.64f, -14.98f), Point(0.78f, 0.04f, 0.62f));
	}
END_TEST(TessellatedRanRan)

///////////////////////////////////////////////////////////////////////////////

static const char* gDesc_Bunny = "Bunny scene. Used for the raytracing test.";

IMPLEMENT_STATIC_MESH_SCENE(TessellatedBunny, gDesc_Bunny, "Bunny.bin", 3)
	virtual void	GetSceneParams(PINT_WORLD_CREATE& desc)
	{
		TestBase::GetSceneParams(desc);
		desc.mCamera[0] = CameraPose(Point(11.67f, 16.70f, 11.07f), Point(-0.67f, -0.53f, -0.52f));
	}
END_TEST(TessellatedBunny)
